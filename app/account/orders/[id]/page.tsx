    ...
    <div className="min-h-screen">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <h1 className="font-serif text-3xl md:text-4xl font-bold mb-4">Order #{order.id.slice(0, 8)}</h1>
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <Badge className={getStatusColor(order.shipping_status || "pending")}>
                      {(order.shipping_status || "pending").charAt(0).toUpperCase() +
                        (order.shipping_status || "pending").slice(1)}
                    </Badge>
                    {order.tracking_number && (
                      <p className="text-sm text-muted-foreground mt-1 font-mono">Tracking: {order.tracking_number}</p>
                    )}
                  </div>
                  {order.tracking_number && (
                    <Button variant="outline" size="sm" asChild>
                      <Link href={`/track?tracking=${order.tracking_number}`}>Track Package</Link>
                    </Button>
                  )}
                </div>
                  ...
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center ${
                        ["processing", "shipped", "delivered"].includes(order.shipping_status || "")
                  ...
                  <div className="space-y-4">
                    {order.shipping_updates.map((update, index) => (
                      <div key={update.id}>
                        ...
                        {index < order.shipping_updates.length - 1 && <Separator className="my-4" />}
                      </div>
                    ))}
                  </div>
                </CardContent>
