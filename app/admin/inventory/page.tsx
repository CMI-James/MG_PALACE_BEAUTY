...
export default async function AdminInventory() {
  ...
  const [{ data: products }, { data: lowStockProducts }, { data: outOfStockProducts }, { data: recentLogs }] =
    await Promise.all([
      supabase.from("products").select("*, categories(name)").order("quantity", { ascending: true }).limit(20),
      supabase
        .from("products")
      ...
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="py-6">
            ...
            <p className="mt-2 text-gray-600">Monitor and manage your product inventory</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Less than 10 items</p>
            </CardContent>
          </Card>
              ...
              <Card className="border-yellow-200 bg-yellow-50">
                ...
                <CardContent>
                  <div className="space-y-2">
                    ...
                    {lowStockCount > 5 && (
                      <p className="text-sm text-yellow-700">And {lowStockCount - 5} more products...</p>
                    )}
                  </div>
                </CardContent>
              </Card>
                  ...
                  {products?.map((product) => (
                    <div key={product.id} className="flex items-center justify-between p-4 border rounded-lg">
                      ...
                      <div className="text-right space-y-2">
                        <Badge
                          variant={
                            product.quantity === 0 ? "destructive" : product.quantity < 10 ? "secondary" : "default"
                          }
                        >
                        ...
                        <div>
                          <Button variant="outline" size="sm" asChild>
                            <Link href={`/admin/products/${product.id}/edit`}>Adjust</Link>
                          </Button>
                        </div>
                      </div>
                    </div>
                  ...
                  {recentLogs?.map((log) => (
                    <div key={log.id} className="border-l-2 border-primary pl-4">
                      ...
                      <p className="text-sm text-muted-foreground">
                        ...
                        {log.quantity_change} ({log.previous_quantity} → {log.new_quantity})
                      </p>
